...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!item) {
    return { title: 'Product Not Found | Insurance' }
  }

  return {
    title: `${item.title} | Insurance`,
            ...
            <div className="lg:col-span-1">
              <div className="bg-white rounded-xl shadow-sm p-6 sticky top-24">
                ...
                <dl className="space-y-4">
                  {(item as any).coverageType && (
                    <div>
                      <dt className="text-sm text-gray-500">Coverage Type</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).coverageType}</dd>
                    </div>
                  )}
                  {(item as any).premiumRange && (
                    <div>
                      <dt className="text-sm text-gray-500">Premium Range</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).premiumRange}</dd>
                    </div>
                  )}
                </dl>
                <div className="mt-8">
